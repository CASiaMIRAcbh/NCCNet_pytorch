...
class NCC(torch.nn.Module):
    ...
    def __init__(self, template, keep_channels=False):
        ...
        self.normalized_template[torch.logical_not(channel_selector)] = 0
...
def batch_ncc(source_batch, template_batch, eps=1e-7):
    ...
    n_template_batch = (template_batch - tbmean) / tbstd
    
    source_b_std = patch_std(source_batch, (tC, tH, tW))

    patch_elements = torch.Tensor((tC, tH, tW)).prod().item()

    source_b_c = source_batch.view_as(torch.Tensor(sC, sN, sH, sW))
    res_c = F.conv2d(source_b_c, n_template_batch, groups=sN)
    res = res_c.view_as(torch.Tensor(sN, sC, (sH-tH+1), (sW-tW+1)))

    # Try to avoid loss is NaN
    res = res.div_(patch_elements * source_b_std + eps)
    return res